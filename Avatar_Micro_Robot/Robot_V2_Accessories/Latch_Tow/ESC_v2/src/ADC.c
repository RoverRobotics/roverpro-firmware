...
Notes:
	...
	- t_total = t_sample + t_conversion
	          = SAMC*T_AD + (2 * num_AD_bits) * T_AD
	          = SAMC*T_AD + (2 * num_AD_bits) * ((2/f_osc)*(ADCS + 1))
	          = (SAMC + (2 * num_AD_bits)) * ((2/f_osc)*(ADCS + 1))
	          = (31 + (2 * 10)) * ((2/(32MHz))*(5 + 1))
	          ~= 3us
	   t_total*16channels = 306us 
	               
	- for correct A/D conversions, the A/D conversion clock (T_AD) must be 
	  selected to ensure a minimum T_AD time of 75ns.  
	  
	- What is ADCS_min?
	  ADCS_min = T_AD_min / T_CY - 1
	           = 75nanoseconds / (2 / (32MHz)) - 1
	           ~= 1

(see p.274 of datasheet)
T_AD = T_CY*(ADCS + 1)
             ADCS = T_AD / T_CY - 1
where T_AD = A/D conversion clock period
      T_CY = 2/f_osc, instruction cycle period
      ADCS = A/D Conversion Clock select bits = how many periods of T_CY
=============================================================================*/
//#define TEST_ADC
/*---------------------------Dependencies------------------------------------*/
#include "./ADC.h"
...
static void ConfigurePins(unsigned int bit_mask) {
  ...
  AD1CON2bits.BUFM = 0;       // start filling buffer at address, ADCBUF0, and 
	...
	AD1CON3bits.ADCS = 0b00000111;// T_AD = T_CY*(ADCS + 1)
	...
	AD1CON3bits.SAMC = 0b11111;	// configure how often to auto-sample
}
