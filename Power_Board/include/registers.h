...
typedef struct {
    int16_t left, right, flipper;
} MOTOR_DATA_3EL_16BI;

typedef struct {
    int16_t left, right, board;
} TMP_3EL_16BI;
typedef struct {
    int16_t left, right;
} MOTOR_DATA_2EL_32BI;
...
typedef struct {
    int16_t pot1, pot2;
} FLIPPER_DATA_2EL_16BI;
typedef struct {
    int32_t left, right, flipper;
} MOTOR_DATA_3EL_32BI;
typedef struct {
    int8_t left, right, flipper;
} MOTOR_DATA_3EL_8BI;
typedef struct {
    float data[4][3];
} MOTOR_DATA_CTRL;
typedef struct {
    int16_t a, b;
} BATTERY_DATA_2EL_16BI;
typedef struct {
    uint16_t deg, min, sec;
} GPS_VECT;
typedef struct {
    GPS_VECT lat, lon;
} GPS_DATA;
...
typedef struct {
    uint32_t length, magic;
} UPDATE_FIRMWARE; // magic = 0x2345BCDE

typedef struct {
    int16_t tilt, zoom;
} REG_CAMERA_VEL_ROT_2EL_16BI;
// PTZ data type for representing tilt, zoom, and digital zoom data
typedef struct {
    int16_t tilt, zoom, digitalZoom;
} REG_CAMERA_POS_ROT_3EL_16BI;
typedef struct {
    int16_t turret, shoulder, elbow, wrist, gripper;
} ARM_DATA_5EL_16BI;
typedef struct {
    uint16_t turret, shoulder, elbow, wrist, gripper_actuator, gripper;
} ARM_DATA_6EL_16BI;
...
REGISTER(REG_MOTOR_TEMP_STATUS, DEVICE_READ, DEVICE_MOTOR, SYNC,
         TMP_3EL_16BI) // 0-data is bad, 1-data is good
...
REGISTER(REG_HITCH_OPEN, DEVICE_WRITE, DEVICE_HITCH, SYNC,
         hitch_t) // non-zero is software desire to open, zero is software's desire to latch
REGISTER(REG_HITCH_POSITION, DEVICE_READ, DEVICE_HITCH, SYNC,
         hitch_t) // 0 (unlatched) to 100 (latched)
...
REGISTER(REG_INITIATOR_CHARGE, DEVICE_WRITE, DEVICE_INITIATOR, SYNC, uint8_t)
REGISTER(REG_INITIATOR_FIRE, DEVICE_WRITE, DEVICE_INITIATOR, SYNC, uint8_t)
REGISTER(REG_INITIATOR_STATE, DEVICE_READ, DEVICE_INITIATOR, SYNC, uint16_t)
...
REGISTER(REG_BATTERY_TEMP_A, DEVICE_READ, DEVICE_MOTOR, SYNC, uint16_t)
REGISTER(REG_BATTERY_TEMP_B, DEVICE_READ, DEVICE_MOTOR, SYNC, uint16_t)

REGISTER(REG_BATTERY_CURRENT_A, DEVICE_READ, DEVICE_MOTOR, SYNC, int16_t)
REGISTER(REG_BATTERY_CURRENT_B, DEVICE_READ, DEVICE_MOTOR, SYNC, int16_t)
