...
//watchdog timer triggers every 1:06 or so
_CONFIG1( JTAGEN_OFF & GCP_OFF & GWRP_OFF & COE_OFF & FWDTEN_ON & ICS_PGx2 & FWPSA_PR128 & WDTPS_PS16384) 
_CONFIG2( IESO_OFF & FCKSM_CSDCMD & OSCIOFNC_ON & POSCMOD_HS &
          FNOSC_PRIPLL & PLLDIV_DIV5 & IOL1WAY_ON)

/*_CONFIG1( JTAGEN_OFF & GCP_OFF & GWRP_OFF & COE_OFF & FWDTEN_ON & ICS_PGx2 & FWPSA_PR128 & WDTPS_PS1024) 
_CONFIG2( IESO_OFF & FCKSM_CSDCMD & OSCIOFNC_ON & POSCMOD_HS &
          FNOSC_PRIPLL & PLLDIV_DIV5 & IOL1WAY_ON)*/
   ...
   if(!USBHandleBusy(USBGenericOutHandle)) ///< if packet has been received
    {
		...
		while(1)
		{
			//cur_word = *(uint16_t*)(OutPacket + n);   // get register value
			cur_word = (OutPacket[n+1]<<8)+OutPacket[n];
