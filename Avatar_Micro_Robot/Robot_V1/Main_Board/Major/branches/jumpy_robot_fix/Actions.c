...
if(select0 && (return_second_payload_type()==HITCH))
{
	hitch_in_use = 1;
}
else
...
if(select0 && (return_payload_type()==ARM))
{
	...
	{
		arm_in_use = 0;
		//sets desired motor pulses to Drive_System module variables
		//Set_Desired_Motor_Pulse(L_LR,L_UD,Flipper_On,Flipper_Dir);
		if(disable_driving)
