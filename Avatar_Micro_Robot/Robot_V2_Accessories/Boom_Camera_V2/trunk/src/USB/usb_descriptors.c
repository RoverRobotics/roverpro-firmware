...
ROM struct {
	...
	BYTE bDscType;
	WORD string[1];
	} sd000 = {sizeof(sd000),USB_DESCRIPTOR_STRING,{0x0001}
};
...
ROM struct {
	...
	BYTE bDscType;
	WORD string[12];
	} sd001 = {sizeof(sd001), USB_DESCRIPTOR_STRING, {'R','o','b','o','t','e','X',',',' ','I','n','c'}
};
...
ROM struct{
	BYTE bLength;
	BYTE bDscType;
	WORD string[sizeof(L"" __DATE__ " "  __TIME__ )/2];
	} sd002={(BYTE)sizeof(sd002), //the length of each element summed up
