...
static void ConfigureOC1(unsigned int ms);
static void ConfigureOC2(unsigned int ms);
static void InitTimer2(void);

/*---------------------------Module Variables---------------------------------*/
static unsigned char OC1_RPn, OC2_RPn = 0; 			// remappable pin number
static unsigned int OC1_period, OC2_period = 0;	// units of Timer2Ticks
...
void PWM_Init(unsigned char pin1, unsigned char pin2, unsigned int ms) {
  if (pin1) {
    OC1_RPn = pin1;
  ...
  if (pin2) {
	...
	if (OC1_RPn) ConfigureOC1(ms);
	if (OC2_RPn) ConfigureOC2(ms);
}


void inline PWM_UpdateDutyCycle(unsigned char pin, float dutyCycle) {
	if (pin == OC1_RPn) OC1R = (unsigned int)(dutyCycle * (float)OC1_period);
	else if (pin == OC2_RPn) OC2R = (unsigned int)(dutyCycle * (float)OC2_period);
}
...
void PWM_Deinit(void) {
	...
	if (OC1_RPn) PPS_MapPeripheral(OC1_RPn, OUTPUT, FN_NULL);
	if (OC2_RPn) PPS_MapPeripheral(OC2_RPn, OUTPUT, FN_NULL);
