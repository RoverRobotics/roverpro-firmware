...
static void block_ms(uint16_t ms) {
...
void pre_boot(){
	// TRISx = data direction. 1=in; 0=out
	_TRISD3 = 0; // Battery A MOS
    _TRISD2 = 0; // Battery B MOS
    
	if (RCONbits.POR) {
		// if we are powering on, need to slowly bring up power bus
		turn_on_power_bus_hybrid_method();
	} else {
		// otherwise, the system is "warm" and we can just switch
