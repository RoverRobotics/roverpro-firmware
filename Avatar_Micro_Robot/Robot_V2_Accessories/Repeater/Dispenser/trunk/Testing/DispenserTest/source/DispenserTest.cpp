  ...
  for (ssize_t t = 0; t < num_devices; t++) {
    ...
    if (desc.idVendor == RBX_VENDOR_ID) {
      ...
      switch (desc.idProduct) {
        ...
        case 0x15: printf("Repeater Dispenser\r\n"); break;
        default:
...
unsigned char handle_dispenser_message(unsigned char REG_REPEATER_RELEASE)
{
  int repeater_positions_index = GetRegisterIndex(&telemetry::REG_REPEATER_POSITIONS);
  int repeater_release_index = GetRegisterIndex(&telemetry::REG_REPEATER_RELEASE);

    //if release is 0xff, don't open or close repeater
    if(REG_REPEATER_RELEASE == 0xff)
    {
      out_packet[0] = repeater_positions_index;
      out_packet[1] = 0x80;
      out_packet[2] = 0xff;
      out_packet[3] = 0xff;
    }
    ...
    {
      out_packet[0] = repeater_release_index;
      out_packet[1] = 0x00;
