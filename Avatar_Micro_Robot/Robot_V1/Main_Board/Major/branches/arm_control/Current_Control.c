...
//	PI_motor_effort[i] += .2*(float)E_speed[i];
	...
	if(arm_in_use)
	{
		if( ((PI_motor_effort[LEFT_MOTOR] > 0) && (PI_motor_effort[RIGHT_MOTOR] > 0) ) || ((PI_motor_effort[LEFT_MOTOR] < 0) && (PI_motor_effort[RIGHT_MOTOR] < 0) ))
		{
			set_motor_effort(-PI_motor_effort[LEFT_MOTOR],LEFT_MOTOR);
			set_motor_effort(-PI_motor_effort[RIGHT_MOTOR],RIGHT_MOTOR);
		}
		else
		{
			set_motor_effort(PI_motor_effort[LEFT_MOTOR],LEFT_MOTOR);
	...
	{	
			...
			set_motor_effort(PI_motor_effort[LEFT_MOTOR],LEFT_MOTOR);
			set_motor_effort(PI_motor_effort[RIGHT_MOTOR],RIGHT_MOTOR);
			handle_flipper(PI_motor_effort[FLIPPER_MOTOR]);
	}
