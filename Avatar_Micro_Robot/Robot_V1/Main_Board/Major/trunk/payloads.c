...
unsigned char message_to_second_payload[MESSAGE_TO_PAYLOAD_LENGTH];
unsigned int return_CRC(unsigned char* data, unsigned char length);
unsigned char message_from_payload_ready = 0;
unsigned char message_from_second_payload_ready = 0;
unsigned char message_from_payload[MESSAGE_FROM_PAYLOAD_LENGTH];
unsigned char message_from_second_payload[MESSAGE_FROM_PAYLOAD_LENGTH];
unsigned char payload_type = NONE;
unsigned char second_payload_type = NONE;
...
void reset_arm(void);
void send_message_to_second_payload(unsigned char* data_to_send);
void send_hitch_message(unsigned char toggle_up, unsigned char toggle_down);
unsigned char arm_in_use = 0;
unsigned char hitch_in_use = 0;
void send_PTZ_message(unsigned char joystick_h, unsigned char joystick_v, unsigned char left_trigger, unsigned char right_trigger, unsigned char payload_button)
{
...
void send_hitch_message(unsigned char toggle_up, unsigned char toggle_down)
{
	...
	if(toggle_up) data_byte1 |= 0x80;
	if(toggle_down) data_byte1 |= 0x40;


	data_to_send[3] = data_byte1;
	data_to_send[4] = 0x00;

	send_message_to_second_payload(data_to_send);
...
void handle_second_payload_message(void)
{
	...
	send_hitch_message(flipper_up,flipper_down);
...
void send_message_to_second_payload(unsigned char* data_to_send)
{
	...
	message_to_second_payload[0] = 0xFF;
	message_to_second_payload[1] = 0xCC;

	message_to_second_payload[MESSAGE_TO_PAYLOAD_LENGTH-2] = CRC>>8;
	message_to_second_payload[MESSAGE_TO_PAYLOAD_LENGTH-1] = CRC&0xFF;

	

	for(i=0;i<MESSAGE_TO_PAYLOAD_LENGTH-4;i++)
	{
		message_to_second_payload[i+2] = data_to_send[i];
		
	}

	U3TXREG = message_to_second_payload[0];
...
//used for secondary payload (hitch)
void init_uart3(void)
{

	//RP3 is tx to payload
	RPOR1bits.RP3R = 28;
	//RP12 is RX from payload
	RPINR17bits.U3RXR = 12;

	U3MODE = 0x0000;

	//32e6/32/9600-1 = 103.17
	U3BRG = SYSCLK/32/9600-1;
...
void  __attribute__((interrupt, auto_psv)) _U3TXInterrupt(void)
{
	...
	{
		U3TXREG = message_to_second_payload[byte_counter];
	}
...
void  __attribute__((interrupt, auto_psv)) _U3RXInterrupt(void)
{
	...
	//avoid reading in a new message until the last one has been processed
	if(message_from_second_payload_ready == 0)
	{
		switch(message_index)
		{
			case 0x00:
			...
			case MESSAGE_FROM_PAYLOAD_LENGTH-1:
					...
					for(i=0;i<MESSAGE_FROM_PAYLOAD_LENGTH;i++)
					{
						message_from_second_payload[i] = message_buffer[i];
					}
					message_from_second_payload_ready = 1;
					message_index = -1;
