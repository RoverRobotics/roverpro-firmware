...
{
	unsigned char id_data[5] = {0x04,0x05,0x00,0x00,0x00};
...
void  __attribute__((interrupt, auto_psv)) _U2TXInterrupt(void)
{
	...
	if(board_type == ELBOW)
	{
		message_length = ZOOM_TX_LENGTH;
	}	
	else
	{
		message_length = NEXT_PAYLOAD_TX_LENGTH;
	}
	...
	{
		U2TXREG = next_payload_tx[message_index];
	}
	//if the bytes are all sent, don't load TXREG.  This won't trigger the interrupt until TXREG is loaded again in an external function.
	else
	{
